...
const AIScenarioAssistantModal: React.FC<AIScenarioAssistantModalProps> = ({ isOpen, onClose, onAddScenario }) => {
  ...
  const [suggestedScenarios, setSuggestedScenarios] = useState<{ name: string; description: string }[] | null>(null);
  ...
  const handleGenerate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result && result.length > 0) {
        ...
        toast.warning('Não foi possível encontrar cenários para essa descrição. Tente ser mais específico.');
      }
    } catch (err: any) {
  ...
  const handleAddScenario = (scenarioToAdd: { name: string; description: string }) => {
      onAddScenario(scenarioToAdd.name);
      setSuggestedScenarios(currentScenarios => currentScenarios?.filter(s => s.name !== scenarioToAdd.name) || null);
  }
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center z-50 transition-opacity duration-300">
      <div className="bg-gray-800 rounded-lg shadow-2xl p-8 w-full max-w-lg transform transition-all scale-95 opacity-0 animate-fade-in-scale">
        ...
        <p className="text-gray-400 mb-6">Descreva seu personagem e a IA irá sugerir categorias para organizar seus poderes.</p>
            ...
            <div className="space-y-4 max-h-[40vh] overflow-y-auto pr-2">
                {suggestedScenarios.map((scenario, index) => (
                    <div key={index} className="bg-gray-900/50 p-4 rounded-lg border border-gray-700">
                        <h4 className="text-xl font-bold text-purple-300">{scenario.name}</h4>
                        <p className="text-gray-300 mt-2 text-sm">{scenario.description}</p>
                        <div className="flex justify-end mt-4">
                            <button onClick={() => handleAddScenario(scenario)} className="py-1 px-3 bg-purple-600 hover:bg-purple-700 rounded-md text-white font-semibold transition text-sm">
                              Adicionar
                            </button>
                        </div>
                    </div>
